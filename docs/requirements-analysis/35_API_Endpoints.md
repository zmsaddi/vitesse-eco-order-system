...
| `/api/v1/action-hub` | GET | pm, gm, manager | **Phase 6.2 — shipped**. ثلاث أقسام مجمَّعة: `urgentActions` (6 counts: overduePayments، reconciliationDue، pendingCancellations، staleSnapshots، lowStock، incompleteSettings + total)، `recentActivity` (≤ 5 rows، reuse Phase 5.2 listActivity)، `teamCounts` (4 counts، reuse Phase 5.3 loadCounts). Response: `{ scope: "global"\|"team", urgentActions, recentActivity[], teamCounts }`. Scope = `team` للـmanager (self + direct-report drivers)، `global` للـpm/gm. seller/driver/stock_keeper → 403 (والـ page-level enforcePageRole يعيد توجيههم لـ role-home). D-72. |
