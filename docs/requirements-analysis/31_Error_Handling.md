...
| `NOTIFICATION_NOT_OWNER` | هذا الإشعار لا يخصّك | Phase 5.1a `markRead` / future per-row mutations: `notification.user_id !== claims.userId`. Dedicated 403 so the UI can distinguish "wrong user" from a generic forbidden. | 403 |
