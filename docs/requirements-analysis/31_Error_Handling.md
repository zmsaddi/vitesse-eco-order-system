...
| `VALIDATION_FAILED` (money precision — Phase 4.3.1) | المبلغ يجب أن يكون بدقة سنتين (2 decimals max) / الرصيد يجب أن يكون بدقة سنتين | Transfer `amount` or Reconcile `actualBalance` carries more than 2 decimal places (e.g. 0.004, 0.005, 10.004). Rejected at the Zod refine; a service-level defense also rejects `round2(amount) < 0.01` on transfer so no zero-value movement can be inserted by a direct service call. | 400 |
